 ...
 * @return	void
 */
function print_rh_menu($db,$atarget,$type_user)
{
...
/*
	?>
	<a href="<?=DOL_URL_ROOT?>/index.php?mainmenu=home" id="logoSimpleRH"></a>
	<?
*/
	...
	for($i = 0; $i < $num; $i++)
	{
		...
		$showmode=dol_rh_showmenu($type_user,$newTabMenu[$i],$listofmodulesforexternal);
...
function print_start_menu_array()
{
	...
	?><a id="logo-simple-rh" href="<?=DOL_URL_ROOT ?>/index.php?mainmenu=home"></a><?
 ...
 * @return	void
 */
function print_left_rh_menu($db,$menu_array_before,$menu_array_after)
{
    ...
    if ($mainmenu)
    {
        ...
        $tabMenu=array();
        $menuArbo = new Menubase($db,'rh','left');
    ...
    if (is_array($menu_array))
    {
    	...
    	for ($i = 0; $i < $num; $i++)
        {
            ...
            if ($tabul > 0)
            {
                for ($j=0; $j < $tabul; $j++)
                {
                    $tabstring.='<div style="float:left;">&nbsp; &nbsp;</div>';
                }
            }
 ...
 */
function dol_rh_showmenu($type_user, &$menuentry, &$listofmodulesforexternal)
{
